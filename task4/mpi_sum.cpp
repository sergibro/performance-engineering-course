...
unsigned int find_sum0(unsigned char* one_channel, uint32_t channel_size){
  clock_t start_t;
  clock_t end_t;
  clock_t clock_delta;
...
unsigned int find_sum1_inner(unsigned char* one_channel, uint32_t start, uint32_t end) {
...
unsigned int find_sum1(unsigned char* one_channel, uint32_t channel_size, int myid, int num_procs){
  unsigned int sub_sum = 0;
  unsigned int result = 0;
  MPI_Status status;
  uint32_t chunk_size = int(channel_size / num_procs);
  if (myid < num_procs)
    sub_sum = find_sum1_inner(one_channel, myid * chunk_size, (myid + 1) * chunk_size);
  else
    sub_sum = find_sum1_inner(one_channel, myid * chunk_size, channel_size);
  result = sub_sum;
  if (myid == 0) {
    for (int i = 1; i < num_procs; i++) {
      MPI_Recv(&sub_sum, 1, MPI_UINT64_T, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &status);
      result += sub_sum;
    }
  }
  else
    MPI_Send(&sub_sum, 1, MPI_UINT64_T, 0, 1, MPI_COMM_WORLD); 
  return result;
}
...
int main(int argc, char *argv[]){
  ...
  unsigned char r = find_sum1(greenData, channel_size * TIMES, myid, numprocs);
