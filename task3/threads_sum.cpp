...
unsigned char find_sum0(unsigned char* one_channel, uint32_t channel_size){
  clock_t start_t;
  clock_t end_t;
  clock_t clock_delta;
...
void find_min1_inner(uint32_t & result, unsigned char* one_channel, uint32_t start, uint32_t end) {
...
unsigned char find_sum1(unsigned char* one_channel, uint32_t channel_size){
  clock_t start_t;
  clock_t end_t;
  clock_t clock_delta;
  double clock_delta_sec;
  ...
  // for (int j; j < 100000; j++){
    uint32_t *results = (uint32_t *) malloc(N_THREADS * sizeof(uint32_t));
...
int main(){
  ...
  uint32_t r1 = find_sum0(greenData, channel_size * TIMES);
  uint32_t r2 = find_sum1(greenData, channel_size * TIMES);

  printf("SUM is: %i & %i", r1, r2);
